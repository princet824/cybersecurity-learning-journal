# cybersecurity-learning-journal
Documenting my path into cybersecurity-courses, notes, and hands-on practice.
## Background 
-Cisco Intro to Cybersecurity - done
-Cisco Networking Basics - done
## Current Focus
-TryHackMe: Pre Security Path (in progress)
-Professor Messer: CompTIA A+ Core 1 (220-1201)
## Progress Log
Date: 2026-09-23
Topic: Computer Hardware, assembling a PC
Source: TryHackMe (Cold Boot room)
Notes: Learned PSU, RAM, GPU placement and matching specs to a case file
---
Date: 2026-09-23
Topic: Started Professor Messer CompTIA A+Core 1 (220-1201)
Source: Professor Messer (Youtube)
Notes: 
